...
class TrendyolAPIConfig(models.Model):
  ...
  api_secret = models.CharField(max_length=255,
                                help_text="Trendyol API Secret")
  base_url = models.URLField(
      default="https://apigw.trendyol.com",
      help_text="Trendyol API base URL - should be 'https://apigw.trendyol.com'"
  )
  user_agent = models.CharField(
      max_length=150,
      blank=True,
      help_text=
      "User-Agent header for API requests. Format: 'SellerID - SelfIntegration'"
  )
  supplier_id = models.CharField(
      max_length=100,
      blank=True,
      help_text="Trendyol Supplier ID (usually same as Seller ID)")
...
class TrendyolCategory(models.Model):
  ...
  category_id = models.IntegerField(unique=True,
                                    help_text="Trendyol Category ID")
...
class TrendyolProduct(models.Model):
  ...
  product_main_id = models.CharField(max_length=100,
                                     blank=True,
                                     help_text="Main product ID")
  ...
  pim_category_id = models.IntegerField(
      null=True,
      blank=True,
      help_text="Product Information Management Category ID (optional)")
  ...
  # LCWaikiki relation
  lcwaikiki_product = models.ForeignKey(Product,
  ...
  trendyol_id = models.CharField(max_length=50,
                                 blank=True,
                                 help_text="Trendyol product ID")
  trendyol_url = models.URLField(blank=True,
                                 help_text="URL to the product on Trendyol")
  attributes = models.JSONField(default=dict,
                                blank=True,
                                help_text="Product attributes for Trendyol")

  # Synchronization
  batch_id = models.CharField(max_length=100,
                              blank=True,
                              help_text="Batch ID for synchronization")
  batch_status = models.CharField(max_length=20,
                                  choices=BATCH_STATUS_CHOICES,
                                  default='pending',
                                  help_text="Status of the synchronization")
  status_message = models.TextField(blank=True,
                                    help_text="Status or error message")
  last_check_time = models.DateTimeField(null=True,
                                         blank=True,
                                         help_text="Last status check time")
...
class TrendyolAttribute(models.Model):
  """
  Trendyol attribute (öznitelik) model.
  Stores attribute information from Trendyol categories.
  """
...
class TrendyolAttributeValue(models.Model):
  """
  Trendyol attribute value model.
  Stores possible values for attributes.
  """
  attribute = models.ForeignKey(TrendyolAttribute, 
  ...
  def __str__(self):
    return f"{self.name} (ID: {self.value_id})"
