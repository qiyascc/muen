        ...
        try:
            ...
            for cat in self._get_all_leaf_categories(categories):
                if search_term.lower() in cat['name'].lower():
                    logger.info(f"Found matching category: {cat['name']} (ID: {cat['id']})")
                    return cat['id']
...
class TrendyolProductManager:
    ...
    def get_brand_id(self, brand_name: str) -> int:
        ...
        try:
            ...
            if 'LCW' in brand_name or 'LC Waikiki' in brand_name:
                logger.warning(f"Brand not found: {brand_name}, using default LC Waikiki ID: 7651")
                return 7651
    ...
    def _get_attributes_for_category(self, category_id: int) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Failed to get attributes for category {category_id}: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            if lcw_brand:
                ...
                logger.info(f"Found brand from database: {lcw_brand.name} (ID: {brand_id})")
            else:
        ...
        if trendyol_product and trendyol_product.category_id:
            ...
            try:
                ...
                raise ValueError(f"Could not find appropriate category for product: {category_name}")
...
def prepare_product_for_trendyol(trendyol_product: TrendyolProduct) -> Dict:
    ...
    attributes = product_manager._get_attributes_for_category(trendyol_product.category_id)
    ...
    payload = {
        ...
        "salePrice": float(trendyol_product.price),  # Use the same price if no discount
        "vatRate": trendyol_product.vat_rate or 18,
        "images": [{"url": trendyol_product.image_url}],
        "attributes": attributes
    }
...
def check_product_batch_status(trendyol_product: TrendyolProduct) -> Dict:
    ...
    try:
        ...
        logger.error(f"Failed to check batch status for product {trendyol_product.id}: {str(e)}")
        return {"error": str(e)}