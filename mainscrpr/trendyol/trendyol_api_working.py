...
class TrendyolAPI:
  ...
  def __init__(self, api_config=None):
    ...
    if self.config:
      ...
      logger.warning(
          "Initializing with default values - this is likely not what you want"
      )
    ...
    logger.info(
        f"Initialized TrendyolAPI client with base URL: {self.base_url}")
  ...
  def make_request(self, method, endpoint, data=None, params=None):
    ...
    working_endpoints = {
        ...
        "/product/sellers/{seller_id}/products",
    ...
    for pattern, working_endpoint in working_endpoints.items():
      if pattern in endpoint:
        ...
        if "{category_id}" in working_endpoint:
          ...
          if category_id:
            working_endpoint = working_endpoint.replace(
                "{category_id}", str(category_id))
          else:
            logger.warning(
                f"Could not extract category_id for endpoint: {endpoint}")
            continue

        if "{seller_id}" in working_endpoint:
          working_endpoint = working_endpoint.replace("{seller_id}",
                                                      str(self.seller_id))

        logger.info(
            f"Using working endpoint mapping: {endpoint} -> {working_endpoint}"
        )
  ...
  def get_category_attributes(self, category_id):
    ...
    response = self.make_request(
        'GET', f'/product/product-categories/{category_id}/attributes')
    ...
    if isinstance(product_data, dict):
      ...
      if isinstance(product_data, TrendyolProduct):
        items = [{
            ...
            "title":
            product_data.title[:100].strip(),
            "productMainId":
            product_data.product_main_id or product_data.barcode,
            "brandId":
            product_data.brand_id,
            "categoryId":
            product_data.category_id,
            "quantity":
            product_data.quantity or 10,
            "stockCode":
            product_data.stock_code or product_data.barcode,
            "dimensionalWeight":
            1,
            "description": (product_data.description
                            or product_data.title)[:500],
            "currencyType":
            product_data.currency_type or "TRY",
            "listPrice":
            float(product_data.price),
            "salePrice":
            float(product_data.price),
            "vatRate":
            product_data.vat_rate or 20,
            "cargoCompanyId":
            17,
            "shipmentAddressId":
            0,
            ...
            "attributes":
            product_data.attributes or []
        }]
      else:
        raise ValueError(
            f"Unsupported product data type: {type(product_data)}")

    payload = {"items": items}
    response = self.make_request('POST',
                                 f'/product/sellers/{self.seller_id}/products',
                                 data=payload)

    # Update batch ID in product model if it's a Django model
    if 'batchRequestId' in response and isinstance(product_data,
                                                   TrendyolProduct):
  ...
  def get_batch_status(self, batch_id):
    ...
    response = self.make_request(
        'GET',
        f'/product/sellers/{self.seller_id}/products/batch-requests/{batch_id}'
    )
    return response
...
class TrendyolCategoryFinder:
  ...
  def __init__(self, api_client=None):
    ...
    try:
      ...
      logger.warning(
          "sentence-transformers not available, falling back to text matching only"
      )
    ...
    if categories:
      ...
      try:
        ...
        if api_categories:
          ...
          for category in all_categories:
            cat, created = TrendyolCategory.objects.get_or_create(
                ...
                defaults={
                    ...
                    'path':
                    ' > '.join(category.get('path', []))
                    if 'path' in category else None
                })
            if not created:
              cat.name = category['name']
              cat.parent_id = category.get('parentId')
              cat.path = ' > '.join(category.get(
                  'path', [])) if 'path' in category else None
              cat.save()
          ...
          logger.info(
              f"Loaded and saved {len(self.categories)} categories from API")
        else:
    ...
    if self.model and self.categories:
      try:
        ...
        if len(category_texts) > 0:
          ...
          logger.info(
              f"Computed embeddings for {len(self.category_embeddings)} categories"
          )
  ...
  def _get_category_attributes(self, category_id):
    ...
    try:
      ...
      logger.error(
          f"Error getting attributes for category {category_id}: {str(e)}")
      return []
    ...
    for attr in attributes_data:
      ...
      if attribute_id:
        ...
        if 'attributeValues' in attr and attr['attributeValues']:
          ...
          value_id = attr['attributeValues'][0]['id'] if 'id' in attr[
              'attributeValues'][0] else None
          value_name = attr['attributeValues'][0]['name'] if 'name' in attr[
              'attributeValues'][0] else None
          ...
          if value_id is not None:
            ...
            try:
              ...
              logger.warning(
                  f"Could not convert attribute ID or value to integer: {attribute_id}, {value_id}"
              )
              continue
            ...
            logger.debug(
                f"Added sample attribute: {attribute_name} = {value_name} (ID: {value_id})"
            )
          elif allow_custom:
            ...
            logger.debug(
                f"Added custom attribute: {attribute_name} = {custom_value}")
  ...
  def find_best_category(self, product_title, product_description=None):
    ...
    if self.model and self.category_embeddings:
      try:
        ...
        for category_id, category_embedding in self.category_embeddings.items(
          ...
          similarity = util.pytorch_cos_sim(product_embedding,
        ...
        if best_category_id:
          ...
          logger.info(
              f"Found category '{category.name}' with similarity {best_similarity:.2f}"
          )
    ...
    try:
      ...
      if matches:
        ...
        logger.info(
            f"Found category '{category.name}' with {best_match[1]} word matches"
        )
        return category_id
...
class TrendyolProductManager:
  ...
  def __init__(self, api_client=None, category_finder=None):
    ...
    self.api_client = api_client or TrendyolAPI()
    self.category_finder = category_finder or TrendyolCategoryFinder(
        self.api_client)
      ...
      if isinstance(attr, dict):
        ...
        if 'attributeId' in new_attr:
          try:
            ...
            logger.warning(
                f"Could not convert attributeId to integer: {new_attr['attributeId']}"
            )
            continue
        ...
        if 'attributeValueId' in new_attr:
          try:
            ...
            logger.warning(
                f"Could not convert attributeValueId to integer: {new_attr['attributeValueId']}"
            )
            continue
  ...
  def find_brand_id(self, product):
    ...
    try:
      ...
      if brand:
        logger.info(
            f"Using existing brand ID {brand.brand_id} for {brand_name}")
      ...
      if brands:
        ...
        for brand in brands:
          if brand.get('name', '').lower() == brand_name.lower():
            ...
            try:
              TrendyolBrand.objects.create(brand_id=brand['id'],
                                           name=brand['name'])
            except Exception as e:
              logger.error(f"Error saving brand to database: {str(e)}")

            logger.info(
                f"Found exact brand match: {brand['name']} (ID: {brand['id']})"
            )
            return brand['id']
        ...
        logger.info(
            f"Using closest brand match: {brand['name']} (ID: {brand['id']})")
        return brand['id']
    except Exception as e:
  ...
  def create_product(self, product):
    ...
    if not product.attributes:
      attributes = self.category_finder.get_category_attributes_sample(
          category_id)
    else:
    ...
    try:
      payload = self._build_product_payload(product, category_id, brand_id,
                                            attributes)
      ...
      if hasattr(e, 'response') and e.response:
        logger.error(
            f"API Response: {e.response.status_code} - {e.response.text}")
      return None
...
def find_best_category_match(product: TrendyolProduct) -> int:
  ...
  return manager.category_finder.find_best_category(product.title,
                                                    product.description)
  ...
  if product.batch_id:
    ...
    if status == 'COMPLETED':
      if failure_reasons:
        ...
        logger.info(
            f"Product {product.id} batch completed with failures, resubmitting"
        )
...
def batch_process_products(products, process_func, batch_size=10, delay=0.5):
  ...
  for i in tqdm(range(0, total, batch_size)):
    batch = products[i:i + batch_size] if hasattr(
        products, '__getitem__') else products[i:i + batch_size]
  ...
  logger.info(
      f"Processed {total} products: {success_count} successful, {failed_count} failed"
  )
