...
class TrendyolBrandAdmin(ModelAdmin):
  ...
  fieldsets = (
      ("Brand Information", {
          "fields": ("brand_id", "name", "is_active")
      }),
      ("Metadata", {
          "fields": ("created_at", "updated_at")
      }),
...
class TrendyolCategoryAdmin(ModelAdmin):
  ...
  list_display = ('category_id', 'name', 'parent_id', 'is_active',
                  'created_at')
  list_filter = ('is_active', )
  ...
  fieldsets = (
      ("Category Information", {
          "fields": ("category_id", "name", "parent_id", "path", "is_active")
      }),
      ("Metadata", {
          "fields": ("created_at", "updated_at")
      }),
...
class TrendyolProductAdmin(ModelAdmin):
  ...
  list_display = ('title', 'barcode', 'brand_name', 'price', 'quantity',
                  'display_batch_id', 'batch_status', 'last_sync_time')
  ...
  readonly_fields = ('created_at', 'updated_at', 'last_check_time',
                     'last_sync_time', 'display_trendyol_link',
                     'display_batch_id', 'formatted_description',
                     'preview_image')
  ...
  fieldsets = (
      ...
      ("Brand and Category", {
          "fields": ("brand_name", "category_name", "brand_id", "category_id")
      }),
      ("Price and Stock", {
          "fields": ("price", "quantity", "vat_rate", "currency_type")
      }),
      ("Images", {
          "fields": ("image_url", "preview_image", "additional_images")
      }),
      ("LCWaikiki Relation", {
          "fields": ("lcwaikiki_product", )
      }),
      ("Trendyol Information", {
          "fields": (
              "trendyol_id",
              "trendyol_url",
              "display_trendyol_link",
              "attributes",
          )
      }),
      ("Synchronization", {
          "fields": ("display_batch_id", "batch_id", "batch_status",
                     "status_message", "last_check_time", "last_sync_time")
      }),
      ("Metadata", {
          "fields": ("created_at", "updated_at")
      }),
  ...
  def display_trendyol_link(self, obj):
    """
        ...
        """
    if obj.trendyol_url:
      return format_html('<a href="{}" target="_blank">Open in Trendyol</a>',
                         obj.trendyol_url)
  ...
  def display_attributes_link(self, obj):
    ...
    if obj.attributes:
      ...
      return format_html(
          '<a href="{}" target="_blank" class="button">View Attributes</a>',
          attributes_url)
  ...
  actions = [
      'sync_with_trendyol', 'check_sync_status', 'retry_failed_products',
  ...
  def sync_with_trendyol(self, request, queryset):
    ...
    for product in queryset:
      try:
        logger.info(
            f"Processing product '{product.title}', barcode: {product.barcode}"
        )
        ...
        if product.trendyol_id or product.batch_status == 'completed':
          ...
          logger.info(
              f"Product '{product.title}' already exists on Trendyol, updating price and inventory"
          )
          ...
          logger.info(
              f"Product '{product.title}' is new, creating on Trendyol")
    ...
    for product in queryset:
      if product.batch_id:
        try:
          ...
          self.message_user(
              request,
              f"Error checking status for {product.title}: {str(e)}",
              level='error')

    if count:
      self.message_user(request,
                        f"Successfully checked status for {count} products.")
    else:
      self.message_user(request,
                        "No products with batch IDs were found.",
                        level='warning')
  ...
  def retry_failed_products(self, request, queryset):
    ...
    for product in queryset:
      try:
        ...
        try:
          ...
          self.message_user(request,
                            f"API error for {product.title}: {str(e)}",
                            level='error')
      except Exception as e:
        self.message_user(request,
                          f"Error retrying product {product.title}: {str(e)}",
                          level='error')
    ...
    if already_pending_count:
      message_parts.append(
          f"Skipped {already_pending_count} products already in pending/processing state"
      )
    ...
    if message_parts:
      ...
      self.message_user(request,
                        "No products were eligible for retry.",
                        level='warning')
    ...
    for product in queryset:
      try:
        ...
        if not product.lcwaikiki_product_id:
          ...
          if product.barcode:
            matching_product = LCWaikikiProduct.objects.filter(
                product_code=product.barcode).first()

          if not matching_product and product.stock_code:
            matching_product = LCWaikikiProduct.objects.filter(
                product_code=product.stock_code).first()
        ...
        if lcw_product:
          ...
          product.image_url = lcw_product.images[
              0] if lcw_product.images else product.image_url
        ...
        self.message_user(
            request,
            f"Error refreshing product {product.title}: {str(e)}",
            level='error')
    ...
    if obj.additional_images:
      ...
      if additional_urls:
        ...
        for i, img_url in enumerate(
            additional_urls[:5]):  # Limit to first 5 additional images
          if img_url:
            html.append(f"""
  ...
  def formatted_attributes(self, obj):
    ...
    html.append('<tr>')
    html.append(
        '<th style="padding: 8px; border: 1px solid #ddd; text-align: left;">Attribute ID</th>'
    )
    html.append(
        '<th style="padding: 8px; border: 1px solid #ddd; text-align: left;">Attribute Name</th>'
    )
    html.append(
        '<th style="padding: 8px; border: 1px solid #ddd; text-align: left;">Value ID</th>'
    )
    html.append(
    ...
    for attr in attributes:
      try:
        ...
        html.append('<tr>')
        html.append(
            f'<td style="padding: 8px; border: 1px solid #ddd;">{attr_id}</td>'
        )
        html.append(
            f'<td style="padding: 8px; border: 1px solid #ddd;">{attr_name}</td>'
        )
        html.append(
            f'<td style="padding: 8px; border: 1px solid #ddd;">{value_id}</td>'
        )
        html.append(
        ...
        html.append(
            f'<tr><td colspan="4" style="padding: 8px; border: 1px solid #ddd; color: red;">Error parsing attribute: {str(e)}</td></tr>'
        )
